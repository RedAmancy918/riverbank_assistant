...
- 多端 Chat 与视频通话：iOS、macOS 和 Windows 客户端提供本地多用户账号、持久化连续 Chat 和服务端强制的用户历史/附件隔离；会话由独立 Worker 绑定 Hermes Daily profile，支持历史恢复、停止生成、上传文件与 Qwen Image 文生图且不阻塞后台任务。生成图片会立即保存为当前账号的私有对话附件，不依赖供应商临时链接；如果独立图片模型尚未配置而 Hermes 在本机生成了图片，Worker 也会仅从 Daily 受信任缓存目录验证并导入图片，回复中不暴露设备内部路径。应用二级菜单中的“通话”页面通过 WebRTC 双向传输画面和语音，树莓派端复用 Camera Hub 与 PipeWire 六麦阵列，不直接抢占摄像头或 ALSA。账号密码和会话经 Tailscale Serve HTTPS 传输，媒体使用 DTLS-SRTP，不依赖公网中转服务。
...
| RiverBank Call | `0.0.0.0:19734` | 经 Tailscale Serve HTTPS 暴露的账号、Chat、任务与 WebRTC 信令；原始端口不得映射公网 |

...
- 多端 Chat 与视频通话：iOS、macOS 和 Windows 客户端提供持久化连续 Chat，会话由独立 Worker 绑定 Hermes Daily profile，支持历史恢复、停止生成且不阻塞后台任务；应用二级菜单中的“通话”页面通过 WebRTC 双向传输画面和语音，树莓派端复用 Camera Hub 与 PipeWire 六麦阵列，不直接抢占摄像头或 ALSA。默认同时支持可信局域网和同一 Tailscale tailnet，接口使用配对令牌，媒体使用 DTLS-SRTP，不依赖公网中转服务。

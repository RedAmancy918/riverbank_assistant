...
from expression_display_persistent import MixedGlyphFont, split_font_runs, uses_cjk_font


class ExpressionFontFallbackTests(unittest.TestCase):
    def test_latin_font_uses_visual_size_compensation(self) -> None:
        calls = []
        ...
        MixedGlyphFont(PygameStub(), "cjk.ttf", "latin.ttf", 21, latin_scale=1.06)
        self.assertEqual(calls, [("cjk.ttf", 21), ("latin.ttf", 22)])
    ...
    def test_latin_numbers_and_symbols_use_the_latin_font(self) -> None:
        ...
        self.assertEqual(
            split_font_runs("Wi-Fi · Edge OS 0.26.6"),
            [(False, "Wi-Fi · Edge OS 0.26.6")],
        )
    ...
    def test_mixed_status_text_is_split_without_losing_characters(self) -> None:
        source = "已开启 · RiverBank Edge 0.26.6 · 自检 22/22"

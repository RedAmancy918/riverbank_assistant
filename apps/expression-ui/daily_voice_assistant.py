...
from riverbank_agent import EmbeddedHermesRuntime
...
class PersistentVoiceAgentRuntime(EmbeddedHermesRuntime):
    """RiverBank voice fast path backed by the replaceable Agent adapter."""

    def __init__(self) -> None:
        super().__init__(
            toolsets=DAILY_TOOLSETS,
            mcp_discovery=DAILY_MCP_DISCOVERY,
            max_turns=DAILY_AGENT_MAX_TURNS,
            timeout_seconds=DAILY_AGENT_TIMEOUT_SECONDS,
            log=log,
        )


PersistentHermesRuntime = PersistentVoiceAgentRuntime
...
class DailyVoiceAssistant:
    def __init__(self) -> None:
        ...
        self.agent_runtime = PersistentVoiceAgentRuntime()
        self.agent_runtime.clarify_handler = self.handle_agent_clarification
    ...
    def request_barge_in(self, wake: dict) -> bool:
        ...
        if async_loop is not None and async_task is not None:
            try:
                ...
                pass
        agent_interrupted = self.agent_runtime.interrupt()
        ...
        log(
            ...
            f"stage={interrupted_stage} agent_interrupted={agent_interrupted}"
        )
        return True
...
def main() -> int:
    ...
    threading.Thread(
        target=assistant.agent_runtime.warmup,
        name="riverbank-voice-agent-warmup",
        daemon=True,
    ).start()
    threading.Thread(

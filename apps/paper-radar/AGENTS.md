...
8. 检查 candidate JSON 中列出的公司官方来源，检索最近 183 天的官方更新并按发布日期从新到旧排序。机器人来源须持续覆盖 Physical Intelligence、Figure、1X、Skild AI、Unitree、自变量机器人/X Square Robot、Sunday Robotics、AGIBOT/智元机器人和 DYNA Robotics；同时关注 Google DeepMind/Gemini、Qwen、DeepSeek、Anthropic/Claude、xAI/Grok、MiniMax、Z.ai/GLM、OpenAI、Meta AI、Mistral、Moonshot/Kimi、ByteDance Seed、Tencent Hunyuan 等基础模型团队。产业动态总计最多 9 条，每个来源最多收录最新 1 条有实质信息的动态；包括机器人/模型发布、VLM 与多模态模型、视频/3D/世界模型、新架构或训练目标、推理扩展与效率、算法与训练数据、技术报告、平台或硬件能力、重要能力演示、生态合作和真实部署。通用模型动态只有在可能影响机器人感知、推理、规划、世界模型、动作生成、跨本体泛化或端侧效率时才收录，并在摘要中明确说明这种关联属于编辑分析；纯聊天产品改名、价格/API 促销、融资人事、招聘、活动预告和普通营销内容一律排除。若一篇官方技术文章达到候选 10 分或精选 40 分，可进入候选/精选，不要再在同日 `industry_updates` 重复展示。
...
11. 在 Paper Radar 根目录运行 `.venv/bin/python scripts/finalize_run.py data/generated/latest-report.json --candidate-file <candidate_file>`。这个固定入口会检查日期、中文摘要、数量上限、阈值和潜在方法标记，通过后再调用渲染器，同时更新首页、归档和 `/candidates/` 候选页。不得绕过这个入口直接运行 `render.py`。
12. 确认命令成功后，最终回复只报告日期、精选论文数、潜在方法数、特别关注数、全文精读数和网页已更新。

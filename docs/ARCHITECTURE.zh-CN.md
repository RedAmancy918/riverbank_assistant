...
4. 录音结束后再由常驻的 Faster-Whisper Base 池以完整音频生成最终转写；主解码使用固定温度 beam-5，只有结果异常时才允许一次有界回退，避免默认多温度重试造成几十秒长尾；
5. 先匹配音量、相机等本地设备命令；
...
唤醒事件由独立监视线程持续读取，不会因主交互线程正在等待 Whisper、Hermes、Qwen 或 TTS 而停止。忙碌时出现新的硬件唤醒事件，系统会停止旧音频、对 Hermes 发出硬中断、取消进行中的视觉任务，并把最新的唤醒事件作为下一轮录音。Faster-Whisper 底层的本地调用无法安全杀死单个 C++ 解码线程，因此旧解码会在后台自行收尾，但前台不等待它；第二个已预热实例可以直接处理新一轮，旧结果也不会进入气泡或 Hermes。多次连续唤醒时只保留最新一次。
...
为了避免后台服务造成首字抖动，Daily Voice 在 systemd 中使用更高的 CPU/IO 调度权重；Hailo 人脸模型仍以 30 FPS 常驻，只在语音链路竞争资源时临时让行。Daily Agent 默认仅暴露口语场景需要的记忆、任务、视觉和网页等工具，降低每轮工具 schema 开销。

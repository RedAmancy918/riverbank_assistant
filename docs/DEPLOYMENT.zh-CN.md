...
当前设备软件正式名称为 **RiverBank Edge OS**，本文对应 `v0.26.4 beta` 的受管 Linux 系统层安装方式。它不是可直接烧录的 `.img.xz`；镜像路线与分发边界见 `docs/EDGE_OS.zh-CN.md`。
...
sudo systemctl enable --now paper-radar-catchup.timer
...
sudo riverbank-release seal \
  --version 0.26.4 --channel beta --notes "verified RiverBank Edge OS release"
